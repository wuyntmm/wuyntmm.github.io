(function() {
  ...
  // --- НАЛАШТУВАННЯ ПРОКСІ ---
  // Використовуємо corsproxy.io для обходу геоблокування
  var origin_url = 'https://rc.bwa.to/';
  var proxy_prefix = 'https://corsproxy.io/?' + encodeURIComponent(origin_url);

  var Defined = {
    api: 'lampac',
    localhost: proxy_prefix, 
    apn: ''
  };
  ...
  // --- ФІКСАЦІЯ ХОСТА ---
  // Важливо: використовуємо оригінальний домен для внутрішньої логіки плагіна
  ...
  window.rch_nws[hostkey].typeInvoke = function rchtypeInvoke(host, call) {
    if (!window.rch_nws[hostkey].startTypeInvoke) {
      ...
      if (Lampa.Platform.is('android') || Lampa.Platform.is('tizen')) check(true);
        ...
        // Пропускаємо перевірку CORS, бо ми все одно йдемо через проксі
        check(true);
      }
    } else call();
  };
    ...
    window.rch_nws[hostkey].typeInvoke('https://rc.bwa.to', function() {
      ...
      client.on("RchClient", function(rchId, url, data, headers, returnHeaders) {
        ...
        function sendResult(uri, html) {
          // ВІДПРАВКА ДАНИХ ЧЕРЕЗ ПРОКСІ
          // Формуємо повний URL до API BWA і загортаємо в проксі
          var target_api = 'https://rc.bwa.to/rch/' + uri + '?id=' + rchId;
          var proxied_post_url = 'https://corsproxy.io/?' + encodeURIComponent(target_api);
          
          $.ajax({
            url: proxied_post_url,
            type: 'POST',
            data: html,
            async: true,
            cache: false,
        ...
        if (url == 'eval') {
          ...
          console.log('RCH', url);
          // Тут використовуємо network.native, але переконаємося, що URL повний
          // Якщо url не містить http, додаємо Defined.localhost (який вже з проксі)
          var request_url = url;
          if (request_url.indexOf('http') !== 0) {
             request_url = Defined.localhost + request_url; 
          }
          
          network["native"](request_url, result, function(e) {
  ...
  function rchRun(json, call) {
    if (typeof NativeWsClient == 'undefined') {
      // ЗАВАНТАЖЕННЯ СКРИПТА ЧЕРЕЗ ПРОКСІ
      var script_url = 'https://rc.bwa.to/js/nws-client-es5.js?v18112025';
      var proxied_script = 'https://corsproxy.io/?' + encodeURIComponent(script_url);
      
      Lampa.Utils.putScript([proxied_script], function() {}, false, function() {
        rchInvoke(json, call);
      }, true);
    } else {
      rchInvoke(json, call);
  ...
  function component(object) {
    ...
    if (balansers_with_search == undefined) {
      ...
      // Запит списку балансерів через проксі (Defined.localhost вже містить проксі)
    ...
    this.initialize = function() {
	  ...
	  if(object.balanser){
          ...
          // Запит напряму до балансера через проксі
  ...
  if (false) {
    if (Platform.is('browser') && location.host.indexOf("127.0.0.1") !== -1) {
      ...
      $.get('https://wuyntmm.github.io/player-inner/' + element.url);
      return;
    }
